...
async def promote(promt):
    ...
    try:
        ...
        await hellevent.edit("`Pironoted Successfully! Abb nacho bencho💃🕺`")
...
async def ban(bon):
    ...
    if reason:
        ...
        await hellevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **HOGYE NA BAN...AAGYA SAWAAD** [{bon.chat.title}](bon.chat_id) !!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    try:
        ...
        await hellevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Was Unbanned Successfully. AAPKO DUSRA CHANCE DIYA JATA HAI SIR...FIRSE APNI GHATIYA HARKATO KA PRADARSHAN NA KRE....🚶**")
        if BOTLOG:
